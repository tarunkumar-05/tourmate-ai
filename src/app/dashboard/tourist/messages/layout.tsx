  ...
  if (!chats || chats.length === 0) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="text-xs text-gray-400">Chats will appear here when you book an experience and message your guide.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col">
      {chats.map((chat) => {
        ...
        return (
          <Link 
            key={chat.id} 
            href={`/dashboard/tourist/messages/${chat.id}`}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 truncate group-hover:text-gray-700 transition-colors">
                  {chat.lastMessage || `Booking: ${chat.booking?.experience?.title}`}
                </p>
              </div>
            </div>
          </Link>
